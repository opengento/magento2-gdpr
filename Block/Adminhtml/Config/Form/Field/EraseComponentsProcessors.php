...
use Opengento\Gdpr\Block\Adminhtml\Config\Form\Field\Select\EraseProcessors;
...
class EraseComponentsProcessors extends AbstractFieldArray
{
     ...
     *
     * @return \Opengento\Gdpr\Block\Adminhtml\Config\Form\Field\Select\EraseProcessors
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    public function getEraseProcessorsSelectRenderer(): EraseProcessors
    {
        if (!$this->hasData('erase_processors_select_renderer')) {
            $this->setData(
                'erase_processors_select_renderer',
                $this->getLayout()->createBlock(
                    EraseProcessors::class,
                    '',
                    ['data' => ['is_render_to_js_template' => true]]
                )
            );
